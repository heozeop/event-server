...
const successfulDateFilterRequests = new Counter(
  "successful_date_filter_requests",
);
...
const LOCATION_FILTERS = ["seoul", "busan", "incheon", "daegu", "daejeon"];
...
export function locationFilterScenario(data: { token: string }) {
  ...
  const location =
    LOCATION_FILTERS[Math.floor(Math.random() * LOCATION_FILTERS.length)];

  // Make request with location filter
  const response = http.get(`${API_BASE_URL}/events?location=${location}`, {
    headers: {
      Authorization: `Bearer ${token}`,
    },
  });
