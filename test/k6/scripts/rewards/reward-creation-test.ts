...
// Custom metrics
const successfulPointRewardCreations = new Counter(
  "successful_point_reward_creations",
);
const successfulBadgeRewardCreations = new Counter(
  "successful_badge_reward_creations",
);
...
export function pointRewardScenario(data: {
  token: string;
  rewardsData: {
  ...
  const response = http.post(`${API_BASE_URL}/rewards/POINT`, payload, {
    headers: {
  ...
  const success = check(response, {
    ...
    "points value matches": (r) =>
      r.json("points") === pointRewardTemplate.points,
  });
...
export function badgeRewardScenario(data: {
  token: string;
  rewardsData: {
    pointRewards: PointRewardEntity[];
    badgeRewards: BadgeRewardEntity[];
  ...
  const response = http.post(`${API_BASE_URL}/rewards/BADGE`, payload, {
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${token}`,
    },
...
export function couponRewardScenario(data: {
  token: string;
  rewardsData: {
    pointRewards: PointRewardEntity[];
    badgeRewards: BadgeRewardEntity[];
  ...
  const response = http.post(`${API_BASE_URL}/rewards/COUPON`, payload, {
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${token}`,
    },
