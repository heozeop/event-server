...
const successfulUserFilterRequests = new Counter(
  "successful_user_filter_requests",
);
const successfulEventFilterRequests = new Counter(
  "successful_event_filter_requests",
);
...
function isEventRequestArray(
  data: unknown,
): data is RewardRequestResponseDto[] {
  return (
    Array.isArray(data) &&
    (data.length === 0 ||
...
export function noFilterScenario(data: {
  token: string;
  testData: { eventIds: string[]; userIds: string[] };
}) {
  randomSleep(2, 10);

  const { token } = data;

  const response = http.get(`${API_BASE_URL}/events/requests`, {
    headers: {
...
export function userFilterScenario(data: {
  token: string;
  testData: { eventIds: string[]; userIds: string[] };
}) {
  randomSleep(2, 10);
  ...
  const success = check(response, {
    ...
    "items have correct userId": (r) => {
      ...
      return (
        items.length === 0 || items.every((item) => item.userId === userId)
      );
    },
  });
...
export function eventFilterScenario(data: {
  token: string;
  testData: { eventIds: string[]; userIds: string[] };
}) {
  randomSleep(2, 10);
  ...
  const success = check(response, {
    ...
    "items have correct eventId": (r) => {
      ...
      if (!isEventRequestArray(items)) return false;
      return (
        items.length === 0 || items.every((item) => item.event.id === eventId)
      );
    },
  });
