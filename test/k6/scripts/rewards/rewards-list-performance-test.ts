...
const successfulWithFilterRequests = new Counter(
  "successful_with_filter_requests",
);
...
export function noFilterTest(data: { token: string }) {
  ...
  const response = http.get(`${API_BASE_URL}/rewards`, {
    headers: {
...
export function withFilterTest(data: { token: string }) {
  ...
  const response = http.get(`${API_BASE_URL}/rewards?type=${selectedType}`, {
    headers: {
      Authorization: `Bearer ${token}`,
      "Content-Type": "application/json",
    },
  ...
  const success = check(response, {
    ...
    "all items match filter type": (r) => {
      ...
      return (
        rewards.length === 0 ||
        rewards.every((reward) => reward.type === selectedType)
      );
    },
    "response time is under 100ms": (r) => r.timings.duration < 100,
  });
